...
export class AppBone extends BoneBGL<AppState, AppSchema, AppInjectors> {
    ...
    mergeMap((action: ActionBGL<any>) => {
      ...
      return concat(
        ...
        this.beagle.actions$.pipe(
          filter((thisAction: ActionBGL<any>) => thisAction && thisAction.correlations
            && thisAction.correlations.some(c => c.type === 'async')
            && thisAction.correlations.find(c => c.type === 'async').id === asyncId
          ),
          take(1),
  ...
  ini$ = this.beagle.actions$.pipe(
    ...
    mergeMap((initilaize: ActionBGL<void>) => {
      ...
      const ready = this.bone.actions.setReady.create(true, [ini]);
